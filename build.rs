...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //     .build_server(false)
    //     .extern_path("cosmos", "::cosmos_proto")
    //     .extern_path("cosmos_proto", "::cosmos_proto")
    //     .compile(
    //         &["sdk-go/proto/injective/types/v1beta1/account.proto"],
    //         &["sdk-go/proto/injective/types/v1beta1/"],
    //     )?;
    ...
    let temp_sdk_dir = tmp_build_dir.join("injective-sdk");
    ...
    compile_injective_protos_and_services(&temp_sdk_dir);
    Ok(())
}
...
const INJECTIVE_SDK_DIR: &str = "../sdk-go";
...
fn compile_injective_protos_and_services(out_dir: &Path) {
    info!(
        "Compiling injective-sdk .proto files to Rust into '{}'...",
        out_dir.display()
    );

    let root = env!("CARGO_MANIFEST_DIR");
    let sdk_dir = Path::new(INJECTIVE_SDK_DIR);
    ...
    info!("proto_includes_paths: {:?}", proto_includes_paths);

    // Paths
    let proto_paths = [
        // format!("{}/../proto/definitions/mock", root),
        // format!("{}/proto/injective/auction", sdk_dir.display()),
    ...
    tonic_build::configure()
        ...
        .compile(&Vec::<PathBuf>::new(), &includes)
